...
export default class ResourcesMgr
{
    ...
    {
        let mapRes: Map<string,any> = this.m_arrRes.get(type).get(url);
        if(mapRes != null)
        {
            return mapRes;
        }
    ...
    public LoadRes(res: ResStruct,fnCallback: Function): void
    {
        ...
        //如果已经加载过的直接返回资源
        let mapRes: Map<string,any> = that.m_arrRes.get(type).get(url);
        if(mapRes)
        {
    ...
    public LoadResArray(arrResStruct: Array<ResStruct>,fnCallBack?: Function): void
    {
        ...
        for(let resStruct of arrResStruct)
        {
            ...
            that.m_setAutoRelease.has(resStruct.m_iResType + resStruct.m_sUrl);
    ...
    public LoadScene(sceneName: string,fnCallback: Function): void
    {
        ...
        {
            that.LoadScenePreloadRes(sceneName);
        });
    }

    private LoadScenePreloadRes(sceneName: string): void
    {
        //加载完场景后设置一下
    ...
    private OnCompleteCallback(url: string,type: ResType): void
    {
        ...
        {
            fnCallback(this.m_arrRes.get(type).get(url));
        });
